...
class ProcessingConfig:
    ...
    old_vs_new_54: Dict[str, Dict[str, Dict[str, str]]] 
    reverse: bool
    no_remap: bool
...
def create_parser():
    ...
    parser.add_argument('-54', action='store_true', help='Run only the 5.0 fixes.')
    return parser

def fullfix_update():
    folder_path = os.getcwd()
    if "mods" not in folder_path.lower():
        print("=" * 70)
        print("Warning: The current directory is not inside the 3dmigoto 'Mods' folder path.")
        print("This script expects to be run from within a 3DMigoto mods directory structure.")
        print("Current path:", folder_path)
        print("=" * 70)
        print("Do you want to continue anyway? This may cause unexpected results.")
    ...
    print("\nObservation: If Diluc, Ningguang, Xiao, or other character mods are broken after the fix,")
    print("please restore the Blend file inside those mods' folders.")
...
def process_ini_file(ini_file: str, blend_files: List[str], remaps: Dict[str, RemapData], config: ProcessingConfig) -> ProcessingResult:
    ...
    try:
        ...
        if config.reverse:
            if config.fix_54:
                data, log = update_version(data, {v: k for category in config.old_vs_new_54.values() for character in category.values() for k, v in character.items()}, "5.4 (Reverse)", log)            
            if config.fix_44_47:
            ...
            if config.fix_54:
                data, log = update_version(data, config.old_vs_new_54, "5.4", log)
            ...
            for character, hashes in characters.items():
                for old, new in hashes.items():
                    if old == new and old in data:
                        ...
                        log.append(f"\t[FIX {version}] ({category} - {character}): {old} -> {new}")
                        updated = True
    else:
            ...
            if old == new:
                ...
                if version == "4.1" and json_data:
                    try:
                        ...
                        pass
                log.append(f"\t[FIX {version}]: {old} -> {new}" + (f" (Object: {object_name})" if version == "4.1" else ""))
                updated = True
